...
freq_slider = Slider(ax=axfreq,label='Frequency (Hz)',valmin=1,valmax=200,valinit=100,valstep=1)

# Instantiate sample rate slider
axsr = fig.add_axes([0.25, 0.06, 0.63, 0.0225])
sr_slider = Slider(ax=axsr,label="Sample Rate (Hz)",valmin=2,valmax=200,valinit=101,valstep=1)
...
def update(i):
	...
	domain = time_samps(*window,sr_slider.val)
	ax.set_xlim(period*i+(i-1)*T,period*(i+1)+T*i)
	ln.set_data(domain,signal(domain,freq_slider.val)) # Update line
	return ln
